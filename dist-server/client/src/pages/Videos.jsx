...
export default function Videos() {
    ...
    var _g = useState(false), refreshing = _g[0], setRefreshing = _g[1];
    var _h = useState('grid'), viewMode = _h[0], setViewMode = _h[1];
    var t = translations[language];
    var fetchVideos = function () { return __awaiter(_this, void 0, void 0, function () {
        var response, result, videosArray, errorText, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, fetch('https://globalpulse-news-production-31ee.up.railway.app/api/videos')];
                case 1:
                    response = _a.sent();
                ...
                case 2:
                    ...
                    console.log("\u2705 Successfully loaded ".concat(videosArray.length, " videos from backend"));
                    // Log video details for debugging
                    videosArray.forEach(function (video, index) {
                        console.log("\uD83C\uDFA5 Video ".concat(index + 1, ":"), {
                            id: video._id,
                            title: video.title,
                ...
                case 6:
                    error_1 = _a.sent();
                    console.error('❌ Error fetching videos:', error_1);
    ...
    var getVideoImageUrl = function (video) {
    ...
    var filteredVideos = useMemo(function () {
        ...
        var filtered = safeVideos.filter(function (video) {
            var _a;
            if (!video)
                return false;
            var videoTitle = getDisplayText(video.title);
            var matchesSearch = searchQuery === '' ||
                videoTitle.toLowerCase().includes(searchQuery.toLowerCase());
            var matchesCategory = categoryFilter === 'all' ||
                ((_a = video.category) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === categoryFilter.toLowerCase();
            return matchesSearch && matchesCategory;
        });
        if (selectedTab === 'featured') {
            filtered = filtered.filter(function (video) { return video.isFeatured; });
        }
        else if (selectedTab === 'popular') {
            filtered = filtered.sort(function (a, b) { return (b.views || 0) - (a.views || 0); });
        }
        else if (selectedTab === 'trending') {
            filtered = filtered.filter(function (video) { return video.isTrending; });
        }
        return filtered;
    }, [videos, searchQuery, categoryFilter, selectedTab, language]);
    var trendingVideos = useMemo(function () {
        var safeVideos = Array.isArray(videos) ? videos : [];
        return safeVideos
            .filter(function (video) { return video === null || video === void 0 ? void 0 : video.isTrending; })
            .sort(function (a, b) { return ((b === null || b === void 0 ? void 0 : b.views) || 0) - ((a === null || a === void 0 ? void 0 : a.views) || 0); })
            .slice(0, 4);
    }, [videos]);
    var featuredVideos = useMemo(function () {
        var safeVideos = Array.isArray(videos) ? videos : [];
        return safeVideos
            .filter(function (video) { return video === null || video === void 0 ? void 0 : video.isFeatured; })
            .slice(0, 4);
    }, [videos]);
    var handleSeeAllTrending = function () {
        ...
        setTimeout(function () {
            var _a;
            (_a = document.getElementById('videos-section')) === null || _a === void 0 ? void 0 : _a.scrollIntoView({ behavior: 'smooth' });
        }, 100);
    };
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center py-12">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md mx-auto border border-gray-200 dark:border-gray-700 shadow-lg">
              ...
              <Button onClick={fetchVideos} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300 active:scale-95" size="lg">
      ...
      <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-6 sm:mb-8">
          <div className="relative flex-1">
            ...
            <Input type="search" placeholder={t.search} value={searchQuery} onChange={function (e) { return setSearchQuery(e.target.value); }} className="pl-8 sm:pl-10 pr-3 sm:pr-4 py-1.5 sm:py-2 text-xs sm:text-sm border border-gray-300 dark:border-gray-600 focus:border-red-500 rounded-lg transition-all duration-300 bg-white dark:bg-gray-800 w-full"/>
          </div>
          ...
          <div className="flex gap-2">
            <Button variant={viewMode === 'grid' ? 'default' : 'outline'} size="sm" onClick={function () { return setViewMode('grid'); }} className={"px-3 py-2 ".concat(viewMode === 'grid' ? 'bg-red-600 hover:bg-red-700 text-white' : '')}>
              <Grid className="h-4 w-4"/>
            </Button>
            <Button variant={viewMode === 'list' ? 'default' : 'outline'} size="sm" onClick={function () { return setViewMode('list'); }} className={"px-3 py-2 ".concat(viewMode === 'list' ? 'bg-red-600 hover:bg-red-700 text-white' : '')}>
        ...
        {videos.length > 0 && (<div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4 mb-6 sm:mb-8">
            {[
                { icon: BookOpen, color: 'text-gray-600', value: "".concat(videos.length, "+"), label: t.totalVideos },
                { icon: Eye, color: 'text-gray-600', value: "".concat(videos.reduce(function (sum, video) { return sum + (video.views || 0); }, 0).toLocaleString(), "+"), label: t.dailyViews },
                { icon: Users, color: 'text-gray-600', value: "".concat(videos.reduce(function (sum, video) { return sum + (video.likes || 0); }, 0).toLocaleString(), "+"), label: t.viewers },
                { icon: Clock, color: 'text-gray-600', value: '24/7', label: t.updated }
            ].map(function (stat, index) { return (<div key={index} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 sm:p-3 md:p-4 text-center border border-gray-200 dark:border-gray-700">
        ...
        {trendingVideos.length > 0 && (<div className="mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0 mb-4 sm:mb-6">
              ...
              <Button onClick={handleSeeAllTrending} variant="ghost" className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 font-medium rounded-lg px-2 sm:px-3 py-1 transition-all duration-300 active:scale-95 w-full sm:w-auto text-xs sm:text-sm" size="sm">
                {t.seeAll}
                <ArrowRight className="h-3 w-3 ml-1"/>
              </Button>
            </div>
            <div className={"grid gap-3 sm:gap-4 ".concat(viewMode === 'grid'
                ? 'grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'
                : 'grid-cols-1')}>
              {trendingVideos.map(function (video) { return (<VideoCard key={video._id} title={getDisplayText(video.title)} platform={video.platform || 'youtube'} videoUrl={getVideoUrl(video)} views={video.views || 0} createdAt={video.createdAt} language={language} videoId={video._id} thumbnailUrl={getVideoImageUrl(video)} className={viewMode === 'list' ? 'flex-row h-32' : ''}/>); })}
            </div>
          </div>)}

        {/* Featured Section - Only show if we have featured videos */}
        {featuredVideos.length > 0 && (<div className="mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0 mb-4 sm:mb-6">
              <div className="flex items-center space-x-2">
                <Star className="h-4 w-4 sm:h-5 sm:w-5 text-yellow-500"/>
                <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{t.featured}</h2>
              </div>
              <Button onClick={handleSeeAllFeatured} variant="ghost" className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 font-medium rounded-lg px-2 sm:px-3 py-1 transition-all duration-300 active:scale-95 w-full sm:w-auto text-xs sm:text-sm" size="sm">
                {t.seeAll}
                <ArrowRight className="h-3 w-3 ml-1"/>
              </Button>
            </div>
            <div className={"grid gap-3 sm:gap-4 ".concat(viewMode === 'grid'
                ? 'grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'
                : 'grid-cols-1')}>
              {featuredVideos.map(function (video) { return (<VideoCard key={video._id} title={getDisplayText(video.title)} platform={video.platform || 'youtube'} videoUrl={getVideoUrl(video)} views={video.views || 0} createdAt={video.createdAt} language={language} videoId={video._id} thumbnailUrl={getVideoImageUrl(video)} className={viewMode === 'list' ? 'flex-row h-32' : ''}/>); })}
            </div>
          </div>)}
        ...
        <div id="videos-section">
          <Tabs value={selectedTab} onValueChange={handleTabChange} className="mb-4 sm:mb-6">
        ...
        ].map(function (tab) { return (<TabsTrigger key={tab.value} value={tab.value} className="data-[state=active]:bg-red-600 data-[state=active]:text-white rounded-md px-2 sm:px-3 py-1.5 font-medium transition-all duration-300 flex-1 text-xs sm:text-sm m-0.5">
            ...
            {['all', 'featured', 'trending', 'popular'].map(function (tabValue) { return (<TabsContent key={tabValue} value={tabValue} className="mt-0">
                {filteredVideos.length === 0 ? (<div className="text-center py-6 sm:py-8">
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 sm:p-6 border border-gray-200 dark:border-gray-700 max-w-md mx-auto">
                      ...
                      {videos.length === 0 ? (<p className="text-xs text-gray-500 dark:text-gray-400">
                          {language === 'ar'
                  ...
                  </div>) : (<div className={"grid gap-3 sm:gap-4 ".concat(viewMode === 'grid'
                    ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'
                    : 'grid-cols-1')}>
                    {filteredVideos.map(function (video) { return (<VideoCard key={video._id} title={getDisplayText(video.title)} platform={video.platform || 'youtube'} videoUrl={getVideoUrl(video)} views={video.views || 0} createdAt={video.createdAt} language={language} videoId={video._id} thumbnailUrl={getVideoImageUrl(video)} className={viewMode === 'list' ? 'flex-row h-32' : ''}/>); })}
                  </div>)}
              </TabsContent>); })}
          </Tabs>
        </div>
